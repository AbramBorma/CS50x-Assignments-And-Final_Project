...
def buy():
    ...
    if request.method == 'POST':
        ...
        db.execute("INSERT INTO transactions (user_id, Transaction_Type, Symbol, Shares, Price, Total, DateTime) VALUES (:user_id, :transaction, :symbol, :shares, :price, :total, :date_time)",
                   user_id=user_id, transaction=transaction, symbol=stock_name, shares=shares, price=usd(stock_price), total=total, date_time=date_time)
        ...
        db.execute("UPDATE users SET cash = :cash WHERE id = :user_id",
                   cash=remaining_cash, user_id=user_id)  # updating the users table
...
def history():
    ...
    # Getting the User's Transcations History
    transactions = db.execute(
        "SELECT Transaction_Type, Symbol, Shares, Price, Total, DateTime FROM transactions WHERE user_id = ? ORDER BY DateTime DESC", (user_id,))
...
def change():
    ...
    if request.method == 'POST':
        ...
        db.execute("UPDATE users SET hash = :new_hashed WHERE username = :user",
                   new_hashed=new_hashed, user=user)

        flash("Congratulations! Your password has been changed. Please login again!")
...
def sell():
    ...
    user_id = session.get("user_id")
    info = db.execute("SELECT Symbol, SUM(Shares) AS TotalShares FROM transactions WHERE user_id = ? GROUP BY Symbol", (user_id,))
    cash = db.execute("SELECT cash FROM users WHERE id = :user_id", user_id=user_id)[0]['cash']
    ...
    if request.method == 'POST':
        ...
        db.execute("INSERT INTO transactions (user_id, Transaction_Type, Symbol, Shares, Price, Total, DateTime) VALUES (:user_id, :transaction, :symbol, :shares, :price, :total, :date_time)",
                   user_id=user_id, transaction=transaction, symbol=stock_name, shares=sold_shares, price=usd(stock_price), total=total, date_time=date_time)

        remaining_cash = total + cash

        db.execute("UPDATE users SET cash = :cash WHERE id = :user_id",
                   cash=remaining_cash, user_id=user_id)
